...
function mydisplay3(hObject, eventdata, handles)
   ...
   plot(handles.accel_chunk(:,2),'k')
    hold on
    plot(handles.mech_behaviours,'r')
    plot(handles.eco_behaviours, 'g')
    hold off
    vline(time_sec)
    guidata(hObject, handles)
...
function pushbutton2_accel_Callback(hObject, eventdata, handles)
    ...
    try
        ...
        if width(T) > 4
            handles.accel_chunk = table2array(T(:, 1:7));
            handles.meta = T(:, end-2:end);
        else
            handles.accel_chunk = table2array(T);
            handles.meta = [];
        end
        loaded = true;
    catch e
    ...
    if size(handles.accel_chunk, 2) < 4
        errordlg('Accelerometer data must have at least 4 columns (time, x, y, z)', 'Invalid Data');
        return;
    end

    try
        set(handles.edit2, 'String', datestr(handles.accel_chunk(1,1)));
    catch
        set(handles.edit2, 'String', 'Unknown Date');
    end

    % Create behaviour vectors — seed mech from col 7 if it exists
    n = size(handles.accel_chunk, 1);
    handles.eco_behaviours = zeros(n, 1);
    if size(handles.accel_chunk, 2) >= 7 && any(~isnan(handles.accel_chunk(:,7)))
        handles.mech_behaviours = handles.accel_chunk(:,7);
    else
...
function tag_behaviour(hObject, eventdata, handles, behnum, behType)
    ...
    % If the behaviour column (col 7) exists in raw data, then display it
    if size(handles.accel_chunk, 2) < 7
        handles.data(:, 7) = NaN;
    end
    ...
    switch behType
        ...
        case 'mech'
            ...
            handles.data(idx, 6) = behnum;
    end

    guidata(hObject, handles);
    mydisplay3(hObject, eventdata, handles);

% --- Execute the loading and tagging for 2 types -------------------------
% --- Load behaviours
...
function pushbutton_save_Callback(hObject, eventdata, handles)
    ...
    if isempty(handles.meta)
        tableout = table(time, x, y, z, ID, ...
            handles.eco_behaviours(:), ...
            handles.mech_behaviours(:), ...
            'VariableNames', {'Time','X','Y','Z','ID','eco_behaviour','mech_behaviour'});
    else
        % Remove old behaviour columns from meta if they exist
        drop = ismember(handles.meta.Properties.VariableNames, {'eco_behaviour','mech_behaviour'});
        meta_clean = handles.meta(:, ~drop);
