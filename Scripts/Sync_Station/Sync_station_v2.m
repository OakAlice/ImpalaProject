...
function pushbutton2_accel_Callback(hObject, eventdata, handles)
    ...
    try
        ...
        if width(T) > 4
            handles.accel_chunk = table2array(T(:, 1:5));
    ...
    % Create behaviour vectors — seed mech from col 5 if it exists
    n = size(handles.accel_chunk, 1);
    handles.eco_behaviours = zeros(n, 1);
    if size(handles.accel_chunk, 2) >= 5 && any(~isnan(handles.accel_chunk(:,5)))
        handles.mech_behaviours = handles.accel_chunk(:,5);
    else
...
function tag_behaviour(hObject, eventdata, handles, behnum, behType)
    ...
    % If the behaviour column (col 5) exists in raw data, then display it
    if size(handles.accel_chunk, 2) < 5
        handles.data(:, 5) = NaN;
    end
